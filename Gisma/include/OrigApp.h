...
private:
	...
	long long app_max_iter; // AStar 最大迭代次数; <=0 表示无限(精确). 截断后返回当前 upper_bound(近似)
	ui overall_lb;          // AStar 结束时的有效下界读出（仅读出，不改变搜索行为）

public:
	Application(ui _verify_upper_bound, const char *lower_bound, long long _app_max_iter = 0) ;
	~Application() ;
