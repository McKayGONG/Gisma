...
#include "Utility.h"
#include "ReuseSearchTree.h"     // 放在紧跟 Application.h 之后
...
static std::atomic<long long> g_apptest_nodes{0};
static std::atomic<long long> g_apptest_calls{0};
static std::atomic<long long> g_appforcomp_nodes{0};
static std::atomic<long long> g_appforcomp_calls{0};

struct NodeCountPrinter {
    ~NodeCountPrinter() {
        long long at_n = g_apptest_nodes.load();
        long long at_c = g_apptest_calls.load();
        long long af_n = g_appforcomp_nodes.load();
        long long af_c = g_appforcomp_calls.load();
        if (at_c > 0) {
            fprintf(stderr, "[NODECOUNT] App_test: totalNodes=%lld calls=%lld avg=%.3f\n",
                    at_n, at_c, at_c ? (double)at_n / (double)at_c : 0.0);
        }
        if (af_c > 0) {
            fprintf(stderr, "[NODECOUNT] AppForComputation: totalNodes=%lld calls=%lld avg=%.3f\n",
                    af_n, af_c, af_c ? (double)af_n / (double)af_c : 0.0);
        }
        fflush(stderr);
    }
};
static NodeCountPrinter g_node_count_printer;

// ========== Profiling counters ==========
int Application::prof_phase = 4;
bool Application::disable_lsa_layer = true;   // 默认层关闭（实测死重量; --lsa_layer 1 开回）
size_t Application::prof_h1_count[5] = {0};
size_t Application::prof_h0_count[5] = {0};
size_t Application::prof_lsa_count[5] = {0};
double Application::prof_h1_us[5] = {0};
double Application::prof_h0_us[5] = {0};
double Application::prof_lsa_us[5] = {0};
size_t Application::prof_reuse_calls = 0;
size_t Application::prof_reuse_snapshot_nodes = 0;
double Application::prof_reuse_init_us = 0;
double Application::prof_reuse_rebuild_us = 0;
double Application::prof_reuse_refresh_us = 0;
double Application::prof_reuse_leaves_us = 0;
double Application::prof_reuse_astar_us = 0;
double Application::prof_reuse_astar_pop_us = 0;
double Application::prof_reuse_astar_recompute_us = 0;
double Application::prof_reuse_astar_sibling_us = 0;
double Application::prof_reuse_astar_child_us = 0;
double Application::prof_reuse_astar_bookkeep_us = 0;
size_t Application::prof_reuse_snap_leaves = 0;
size_t Application::prof_reuse_leaves_pruned = 0;
size_t Application::prof_reuse_leaves_to_heap = 0;
size_t Application::prof_reuse_leaves_boundary = 0;
size_t Application::prof_reuse_leaves_invalid = 0;
size_t Application::prof_reuse_astar_iter = 0;
size_t Application::prof_reuse_astar_recomp_pruned = 0;
size_t Application::prof_reuse_astar_exhausted = 0;
int Application::last_reuse_exit_reason = 0;
size_t Application::prof_recompute_calls = 0;
size_t Application::prof_recompute_target_total_sum = 0;
size_t Application::prof_recompute_iters_sum = 0;
size_t Application::prof_recompute_n_sum = 0;
size_t Application::prof_recompute_found_sum = 0;
size_t Application::prof_recompute_typeB_sum = 0;
size_t Application::prof_recompute_break_lb = 0;
size_t Application::prof_recompute_break_intersect = 0;
size_t Application::prof_recompute_break_loop_end = 0;
...
void astar_trace_node(State* state, State* parent, const ui* MO, bool is_expanded, bool is_pruned, bool is_complete) {
    ...
    *g_astar_trace_file << "{\"id\":" << node_id
    ...
    // 输出映射 [q_node, g_node]
    // path中包含所有节点，每个节点的level表示已经映射了level+1个查询节点
    bool first = true;
    for (size_t i = 0; i < path.size(); i++) {
        // path[i]的level表示这个节点映射的是第level个查询节点（0-indexed）
        // 即映射MO[level]到path[i]->image
...
Application::Application(ui _verify_upper_bound, const char *lower_bound, int _app_max_iter, int _exact_max_iter) {
	...
	// 1. 容器初始化（这些是 vector，会自动初始化为空，但显式清空更安全）
	open_heap.clear();
	...
	// 3. 其他可能的成员（根据您的代码可能还有）
	// mapping_found（如果有这个成员）
	// best_mapping（如果有这个成员）
...
Application::~Application() {
	...
	// ========== 新增：清理容器 ==========
	// 这些容器包含的是指针，但指向的对象由内存池管理
	// 所以只需要清空容器本身
	open_heap.clear();
	full_mapping_nodes.clear();
    boundary_nodes.clear();
	
	// 清空池向量（虽然内存已经通过 states_memory 和 siblings_memory 释放了）
	states_pool.clear();
	siblings_pool.clear();
...
void Application::init(const Graph *g, const Graph *q) {
    search_snapshot.clear();          // 清理之前的search_snapshot状态
    open_heap.clear();                // 清理搜索状态容器
    full_mapping_nodes.clear();
    boundary_nodes.clear();
    states_pool.clear();              // 清理内存池
    siblings_pool.clear();
    for(ui i = 0; i < states_memory.size(); i++) {
        delete[] states_memory[i];
    }
    states_memory.clear();            // 清理内存管理容器
    for(ui i = 0; i < siblings_memory.size(); i++) {
        delete[] siblings_memory[i];
    }
    siblings_memory.clear();
...
void Application::init(const std::vector<std::pair<int,int> > &g_v, const std::vector<std::pair<std::pair<int,int>,int> > &g_e, const std::vector<std::pair<int,int> > &q_v, const std::vector<std::pair<std::pair<int,int>,int> > &q_e) {
	search_snapshot.clear();          // * 关键：每次新搜索前清空
	q_n = q_v.size();
	q_starts = new ui[q_n+1]; q_vlabels = new ui[q_n];
...
void Application::preprocess() {
	...
	// children 懒分配可能因 g_n 变大而溢出，这里释放旧的
...
ui Application::App(std::vector<std::pair<ui,ui>> *mapping_ptr, int *lb_ptr)
{
    ...
    // 释放上次 App() 留下的外部引用（cs_cnt 计入），清空容器
    ...
    // 加入外部 list 时给 cs_cnt 加 1，避免 ancestor 在 cascade 时被 recycle。
    // 取代旧的 protect_ancestor_chain（O(depth) 走父链）—— 仅 O(1) 自增。
    // ancestor 链由现有 cs_cnt cascade 自然保活。
    ...
    // 为根节点计算LSa（lazy_parent_lsa=true 時跳過，延後到 extract_snapshot）
#ifdef _USE_LSa_ESTIMATE_BMao_
    if (root->image < g_n && !lazy_parent_lsa) {
        if (!disable_lsa_pruning || !disable_reuse_lsa) {
    ...
    while (heap_n > 0 && open_heap[0]->lower_bound < upper_bound && (exact_value_mode || verify_upper_bound == INF || upper_bound > verify_upper_bound))
    {
        cnt++;
        ...
        // 为兄弟节点计算LSa（lazy_parent_lsa=true 時跳過）
#ifdef _USE_LSa_ESTIMATE_BMao_
        if (sib->image < g_n && !lazy_parent_lsa) {
            if (!disable_lsa_pruning || !disable_reuse_lsa) {
        ...
        if (now->level + 1 == search_n) {
            ...
            // 为子节点计算LSa（lazy_parent_lsa=true 時跳過）
#ifdef _USE_LSa_ESTIMATE_BMao_
            if (child->image < g_n && !lazy_parent_lsa) {
                if (!disable_lsa_pruning || !disable_reuse_lsa) {
...
ui Application::AStar_baseline(vector<pair<ui,ui> > *mapping_ptr, int *lb_ptr) {
	...
	if (trace_env) {
		...
		// 禁用verify_upper_bound限制，让搜索完整执行以生成完整的trace
	...
	while(heap_n > 0 && heap[0]->lower_bound < upper_bound &&
		...
		construct_sibling_baseline(NULL, sibling);  // 使用baseline版本
#else
		construct_sibling_baseline(now, sibling);   // 使用baseline版本
		++now->cs_cnt;
#endif
...
ui Application::App_test(vector<pair<ui,ui> > *mapping_ptr, int *lb_ptr) {
    ...
    // 不再在这里更新overall_lb，因为root会被加入heap
    ...
    while(heap_n > 0 && heap[0]->lower_bound < upper_bound
        ...
        if(now->level+1 == search_n) {
            ...
            // extend_to_full_mapping 会调用 compute_mapped_cost_and_upper_bound
            // 那里会更新 overall_ub
        }
        else {
            // LSa-as-layer: 原 #define _USE_LSa_AS_LAYER_ 改为 runtime 开关（默认开，与宏时期一致）。
            bool lsa_layer_pruned = false;
            if (!disable_lsa_layer) {
    ...
    // ========== 设置最终的overall_lb ==========
    if(heap_n > 0) {
        overall_lb = heap[0]->lower_bound;  // 活跃前沿
    } else {
        overall_lb = terminal_frontier_min_lb;  // 终止前沿
    }
...
ui Application::AppForComputation(vector<pair<ui,ui> > *mapping_ptr, int *lb_ptr) {
    ...
    int cnt = 0;
    exact_hit_iter_limit = false;  // 记号: 每次调用重置(同一 Application 可能被复用)

    // 移除 verify_upper_bound 提前停止条件，确保找到真正的最优解
    while(heap_n > 0 && heap[0]->lower_bound < upper_bound) {

        cnt++;
        if(cnt >= exact_max_iter) {
            // 达到计数限制，标记为非精确解
            exact_hit_iter_limit = true;  // 记号: 露给调用方(见 hit_iter_limit())
            break;
        }
...
void Application::add_to_pool(State *st) {
    // cs_cnt-only protection: external list refs (open_heap, boundary_nodes,
    // full_mapping_nodes) bump cs_cnt. As long as cs_cnt > 0, this caller path
    // shouldn't be reached because callers only invoke add_to_pool when cs_cnt == 0.
...
void Application::add_to_heap(State *st, ui &heap_n, vector<State*> &heap) {
	...
	// 注: 原本这里对每个进堆节点 push_back 到 search_snapshot, 但 search_snapshot
	// 只被 clear/push 从未被读取(extract_snapshot 从 State 树现场构建快照, 不用它),
	// 是死代码且在 A* 热路径上每节点 push+周期性 realloc, 拖慢所有 verify。已移除。
}
...
bool Application::state_has_higher_priority(State* a, State* b) {
	...
	// 深度优先策略：level -> lower_bound -> need_recompute -> image -> mapped_cost
	...
	return a < b;
#else
	// 默认策略：lower_bound -> level -> need_recompute -> image -> mapped_cost
	if(a->lower_bound != b->lower_bound) return a->lower_bound < b->lower_bound;
	if(a->level != b->level) return a->level > b->level;
	if(a->need_recompute != b->need_recompute) return !a->need_recompute && b->need_recompute;
	// Additional tie-breakers for deterministic ordering
...
void Application::generate_best_extension(State *parent, State *now)
{
    ...
    /* ---------- 2. 标记已用 image（跳过 dummy-root） ---------- */
    while (parent != nullptr) {
        if (parent->image < g_n)         /* * 仅真实节点才标记 */
            visY[parent->image] = 1;
...
void Application::compute_best_extension_LSa_baseline(State *now,
                                             ui n,             // candidates[] 数量
                                             ui *candidates,   // 候选 G 顶点 id 列表
                                             ui pre_siblings)  // (若展开兄弟) 已处理 sibling 数
{
/* ------------------------------------------------------------ *
 * 0. Debug：辅助数组必须"全零"
 * ------------------------------------------------------------ */
 ...
 * 2. 临时数组 sizeX / sizeY
 * ------------------------------------------------------------ */
    ui *sizeX = queue;   // 长度 ≥ q_n
    ui *sizeY = prev;    // 长度 ≥ g_n
...
/* ------------------------------------------------------------ *
 * 4. 标记 G 端已占用顶点到 visY / my
 * ------------------------------------------------------------ */
    for (State *parent = now->parent; parent; parent = parent->parent) {
        if (parent->image >= g_n) continue;              /* * 跳过 dummy */
        visY[parent->image] = 1;
        my[parent->image]  = MO[parent->level];
    }

/* ------------------------------------------------------------ *
 * 5. 针对祖先映射计算 sizeY & elabels_matrix 修正
 * ------------------------------------------------------------ */
    ui ged_common = 0;
    for (State *st = now->parent; st; st = st->parent) {
        if (st->image >= g_n) continue;                  /* * 跳过 dummy */
        ui v = st->image;
        ui common = 0;
        sizeY[v] = 0;
...
// 在 compute_best_extension_LSa 的第 14 步之前添加：

#ifdef _EXPAND_ALL_
    /* 14. 若展开所有兄弟，填 siblings[] */
...
#if USE_SIBLING_INTERSECTION && INTERSECTION_WITH_FULL_SNAPSHOT
    // Phase 2 fix: mirror the BMao extension (see line ~3459). App()'s construct_sibling
    // reads now->siblings[intersection_start + tn - 2] where intersection_start =
    // siblings_total_n - siblings_n. Without setting siblings_total_n here the LSa path
    // inherits a stale value from the recycled State node, indexing out of bounds ->
    // garbage image -> compute_mapped_cost segfault. At initial generation both are equal,
    // so intersection_start == 0 (identical to App_test's plain construct_sibling_test).
...
void Application::compute_best_extension_LSa(State *now,
                                             ...
                                             ui pre_siblings)  // (若展开兄弟) 已处理 sibling 数
{
    // The original in-place LSa extension below is DISABLED (known segfault).
    // E8 fix: delegate to the proven-correct baseline LSa implementation, which
    // App_test()/AppForComputation()/AStar_baseline() already use for lb_method==LSa.
    // This only affects App()/construct_sibling() when lb_method==LSa (app-lsa / gisma-lsa);
    // the default BMao App path never reaches here, so main results are unchanged.
    compute_best_extension_LSa_baseline(now, n, candidates, pre_siblings);
    return;
/* ------------------------------------------------------------ *
 * 0. Debug：辅助数组必须"全零"
 * ------------------------------------------------------------ */
...
void Application::compute_best_extension_BM_baseline(char anchor_aware, State *now, ui n, ui *candidates, ui pre_siblings, char no_siblings, char IS) {
	...
	for(ui i = 0;i < n;i ++) {
		...
		ui u = MO[start_idx+i];
		ui u_degree = 0, u_anchored_count = 0, u_non_anchored = 0;
		ui anchored_edges = 0;
		if (all_edge_labels_same) {
			u_degree = q_starts[u+1] - q_starts[u];
			if (anchor_aware) {
				for (ui j = q_starts[u]; j < q_starts[u+1]; ++j) {
					if (visX[q_edges[j]]) u_anchored_count++;
				}
			}
			u_non_anchored = u_degree - u_anchored_count;
		} else {
			for(ui j = q_starts[u];j < q_starts[u+1];j ++) {
...
// #define TIME_PROFILE       // 取消注释以启用时间统计
...
void Application::compute_best_extension_BM_test(char anchor_aware, State *now, ui n, ui *candidates, ui pre_siblings, char no_siblings, char IS) {
    ...
    if (all_edge_labels_same) printf("  [Optimized: all_edge_labels_same enabled]\n");
                                            ...
                                            char IS)  // 函数签名完全不变！
{
    ...
    for (State *p = now->parent; p != NULL; p = p->parent) {
        if (p->image >= g_n) continue;            // * 跳过 dummy-root
        visY[p->image] = 1;
    ...
    /* ---------- 6. 根据类成员margin进行剪枝判断 ---------- */
    // 使用 this->margin（或直接 margin，因为在成员函数中）
    // margin=0时: 使用baseline的严格条件 (>=)
    // margin>0时: 放宽条件，允许保留更多节点
    bool should_prune = false;
    
    if (margin == 0) {
        // baseline版本的行为：>= upper_bound 或 > verify_upper_bound
    ...
    // Intersection-aware early break：lazy_recompute_target_siblings 非 null 時，
    // 找到所有 target image (含 self) 就 break，省掉純 best-only siblings 的 H(0)。
    ui found_in_target = 0;
    ui target_total    = lazy_recompute_target_siblings ? (ui)lazy_recompute_target_siblings->size() : 0;
    if (target_total > 0 && lazy_recompute_target_siblings->count(now->image)) {
        found_in_target = 1;
    }
    ui iters_done = 0;     // count of H(0) iterations actually run (for profiling)
    ui typeB_count = 0;    // siblings computed but NOT in target (intersect-aware waste)
    int break_reason = 0;  // 0=loop end (no break), 1=_EARLY_STOP_ (lb), 2=intersection full
    ...
    for (ui i = 1; i < n; ++i) {
...
#ifdef _EARLY_STOP_
        // 对兄弟节点也应用相同的margin策略（使用类成员margin）
        bool sibling_should_prune = false;

        if (early_stop_at_tau) {
            // 實驗模式：lb > tau 就停（更激進，省 boundary sibling 計算）
            sibling_should_prune = (lb > verify_upper_bound);
        } else if (margin == 0) {
        ...
        if (sibling_should_prune) { break_reason = 1; break; }
#endif

        ui sib_img = candidates[mx[0]];
        t_array[now->siblings_n++] = sib_img;
        t_array[now->siblings_n++] = lb;

        if (target_total > 0) {
            if (lazy_recompute_target_siblings->count(sib_img)) {
                if (++found_in_target == target_total) { break_reason = 2; break; }
            } else {
                ++typeB_count;  // computed sibling that will be dropped by intersection
            }
        }
    }

    // Profile: only count when triggered with target (recompute path)
    if (lazy_recompute_target_siblings) {
        ++prof_recompute_calls;
        prof_recompute_target_total_sum += target_total;
        prof_recompute_iters_sum        += iters_done;
        prof_recompute_n_sum            += n;
        prof_recompute_found_sum        += found_in_target;
        prof_recompute_typeB_sum        += typeB_count;
        if      (break_reason == 1) ++prof_recompute_break_lb;
        else if (break_reason == 2) ++prof_recompute_break_intersect;
        else                        ++prof_recompute_break_loop_end;
    }
...
#if USE_SIBLING_INTERSECTION && INTERSECTION_WITH_FULL_SNAPSHOT
    now->siblings_total_n = now->siblings_n;  // 初始生成时，两者相同
#endif
#endif
}
...
/* ------------------------------------------------------------------
 *  compute_mapped_cost_and_upper_bound  ——  计算 now 的 mapped_cost，
 *  并根据 mapping[] 更新全局 upper_bound / BX。
 * ------------------------------------------------------------------*/
void Application::compute_mapped_cost_and_upper_bound(
        State *now, ui n, ui *candidates, int *mapping) {
    const ui DUMMY = UINT32_MAX;                // queue 未写标记

    /* 0. 清零临时数组（queue/visX/visY） */
    std::fill(queue, queue + g_n, DUMMY);
    std::memset(visX , 0, sizeof(char)*g_n);
    std::memset(visY , 0, sizeof(char)*g_n);

    ui cot         = now->mapped_cost;          // 累积 cost
    ui start_level = now->level;

    /* 1. 把祖先映射写入 queue / visY（跳过 dummy-root） */
    ...
    for (ui i = 0; i < n; ++i) {
        ...
        for (ui j = g_starts[v]; j < g_starts[v+1]; ++j) {
            ui nb = g_edges[j];
            if (!visY[nb]) continue;            // 只关心跨向已映射 G 顶点

            ++cot;
            ui q_nb = queue[nb];
            if (q_nb != DUMMY && q_nb < q_n) {  // queue 中有对应 Q 顶点
    ...
    /* 3. 用完后再次清理 queue / vis 标记（保持全零不破坏后续调用） */
    std::fill(queue, queue + g_n, DUMMY);
    std::memset(visX , 0, sizeof(char)*g_n);
    std::memset(visY , 0, sizeof(char)*g_n);

    /* 4. 更新 overall_ub（无条件更新，记录真正的最优解） */
    ...
    /* 5. 若得到更好的 GED（相对于阈值），则写 BX 并收紧 upper_bound */
#ifdef _UPPER_BOUND_
    if (cot < upper_bound) {
        upper_bound = cot;

        /* 5-A. 祖先部分写 BX，跳过 dummy-root */
        for (State *st = now->parent; st; st = st->parent) {
            if (st->image >= g_n || st->level >= q_n) continue;
            BX[ MO[ st->level ] ] = st->image;
        }

        /* 5-B. 当前分支 mapping 写 BX */
        for (ui i = 0; i + now->level < q_n; ++i)
...
void Application::construct_sibling_baseline(State *pre_sibling, State *now)
{
    ...
    for (State *st = now->parent; st != NULL; st = st->parent)
        if (st->image < g_n) visY[st->image] = 1;          // * 过滤 dummy
    ...
    if (candidates_n <= pre_siblings) {                    // 没新候选
        now->image = g_n;                                  // dummy
        return;
    }
...
#if USE_SIBLING_INTERSECTION && INTERSECTION_WITH_FULL_SNAPSHOT
    // 如果有siblings_total_n，使用它来确定从哪里开始读取
...
void Application::extend_to_full_mapping(State *parent, State *now) {
    ...
    if(lb_method == BMao) {
        ...
        if(q_n < g_n) {
            // 标记已映射的节点
            while(parent != NULL) {
                if (parent->image < g_n)          // * 跳过 dummy-root
                    visY[parent->image] = 1;
                parent = parent->parent;
            }
...
ui Application::Hungarian(char initialization, ui n, ui *cost) {
	...
	if(initialization) prof_h1_count[prof_phase]++; else prof_h0_count[prof_phase]++;
#endif
	...
	double _prof_us = std::chrono::duration_cast<std::chrono::nanoseconds>(_prof_t1 - _prof_t0).count() / 1000.0;
	if(initialization) prof_h1_us[prof_phase] += _prof_us; else prof_h0_us[prof_phase] += _prof_us;
#endif
	return res;
}
...
void Application::verify_induced_ged(State *now) {
    // if (now->image >= g_n || now->level >= q_n) return;   // 跳过 dummy
