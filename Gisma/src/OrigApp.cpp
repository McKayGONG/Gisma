...
Application::Application(ui _verify_upper_bound, const char *lower_bound, long long _app_max_iter) {
...
ui Application::AStar(vector<pair<ui,ui> > *mapping_ptr, int *lb_ptr) {
	...
	while(heap_n > 0&&heap[0]->lower_bound < upper_bound&&(verify_upper_bound == INF||upper_bound > verify_upper_bound)) {
		// app_max_iter 截断: 达到上限则停止 A*, 返回当前 upper_bound (近似, 可能 > 真实 GED)
		if(app_max_iter > 0 && ++_iter > app_max_iter) break;
		State *now = heap[0];
		heap[0] = heap[-- heap_n];
	...
	// 有效下界读出: 堆非空(被 iter 截断)取活跃前沿; 堆空(搜索穷尽)则 upper_bound 即精确 GED。
	overall_lb = (heap_n > 0) ? heap[0]->lower_bound : upper_bound;
