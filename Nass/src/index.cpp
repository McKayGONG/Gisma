...
int NTHREADS = std::max(1, (int)(std::thread::hardware_concurrency() * 0.7));
unsigned MEMLIMIT = 2000; // default memory limit: 2GB (per original paper)
...
int nass::getNextGraphIDLocal()
{
	...
	DataSet* dataset = DataSet::getInstance();
	if(build_timeout || next_graph_id == dataset->numGraphs()){
		id_mutex.unlock();
		return -1;
	}
...
//	bool verbose = true;
	...
	num_indexed++;
	DataSet* dataset = DataSet::getInstance();
	unsigned percent = num_indexed*100/dataset->numGraphs();
	if(percent > progress){
	...
	if(num_indexed == dataset->numGraphs()){
		GETTIME(&ts2);
...
unsigned getRSS()
{
    ...
    if (GetProcessMemoryInfo(GetCurrentProcess(), &pmc, sizeof(pmc)))
        return (unsigned)(pmc.WorkingSetSize / (1024ULL * 1024ULL));
    return 0;
#else
    unsigned rss;
    {
...
void nass::monitor()
{
	...
	while(!done_indexing){
		...
		if(timeout_sec > 0){
			TIMESTAMP now;
			GETTIME(&now);
			long long elapsed = now.tv_sec - ts1.tv_sec;
			if(elapsed >= timeout_sec){
				build_timeout = true;
				if(timeout_sec >= 3600)
					cout << endl << "Index build timeout after " << timeout_sec/3600 << " hours -- CNB (Cannot Build)" << endl;
				else
					cout << endl << "Index build timeout after " << timeout_sec << " seconds -- CNB (Cannot Build)" << endl;
				return;
			}
		}
...
{
	IndexBuildMeta meta;
	buildNassIndex(distributed, address, sampling_rate, meta);
}

void nass::buildNassIndex(bool distributed, char* address, int sampling_rate, const IndexBuildMeta& meta)
{
	...
	GETTIME(&ts1);
	build_timeout = false;
	timeout_sec = (long long)(meta.timeout_hours * 3600);
	...
	GETTIME(&ts2);
	long long build_elapsed = ts2.tv_sec - ts1.tv_sec;

	bool index_valid = false;
	if(!remote && !build_timeout)
		index_valid = loadNassIndex(); // check for index
	else if(remote)
		cout << "\rDONE           " << endl << flush;
	...
	if(!meta.log_file.empty()){
		...
		if(log.is_open()){
			...
			log << "=== Nass Index Build Log ===" << endl;
			log << "Date: " << timebuf << endl;
			log << "Dataset: " << meta.data_file << endl;
			log << "Index file: " << meta.index_file << endl;
			log << "Threshold (tau): " << threshold << endl;
			log << "Num threads: " << NTHREADS << endl;
			log << "Memory limit: " << MEMLIMIT << " MB" << endl;
			if(meta.timeout_hours > 0){
				if(meta.timeout_hours >= 1)
					log << "Time limit: " << (int)meta.timeout_hours << " hours" << endl;
				else
					log << "Time limit: " << (int)(meta.timeout_hours * 3600) << " seconds" << endl;
			} else
				log << "Time limit: none" << endl;
			log << "Num graphs in DB: " << dataset->numGraphs() << endl;
			log << "Graphs indexed: " << num_indexed << " / " << dataset->numGraphs() << endl;
			log << endl;

			if(build_timeout){
				log << "=== Result: CNB (Cannot Build) ===" << endl;
				if(meta.timeout_hours >= 1)
					log << "Reason: Build exceeded " << (int)meta.timeout_hours << "-hour time limit" << endl;
				else
					log << "Reason: Build exceeded " << (int)(meta.timeout_hours * 3600) << "-second time limit" << endl;
				long long h = build_elapsed / 3600;
				long long m = (build_elapsed % 3600) / 60;
				long long s = build_elapsed % 60;
				log << "Elapsed time: " << h << "h " << m << "m " << s << "s" << endl;
			} else {
				log << "=== Result: SUCCESS ===" << endl;
				long long h = build_elapsed / 3600;
				long long m = (build_elapsed % 3600) / 60;
				long long s = build_elapsed % 60;
				log << "Build time: " << h << "h " << m << "m " << s << "s" << endl;

				// index file size
				ifstream idx(meta.index_file, ios::ate | ios::binary);
				if(idx.is_open()){
					long long fsize = idx.tellg();
					idx.close();
					if(fsize < 1024)
						log << "Index size: " << fsize << " B" << endl;
					else if(fsize < 1024*1024)
						log << "Index size: " << fixed << setprecision(1) << fsize/1024.0 << " KB" << endl;
					else if(fsize < 1024LL*1024*1024)
						log << "Index size: " << fixed << setprecision(1) << fsize/(1024.0*1024) << " MB" << endl;
					else
						log << "Index size: " << fixed << setprecision(2) << fsize/(1024.0*1024*1024) << " GB" << endl;
				}

				log << "Index valid: " << (index_valid ? "yes" : "no") << endl;
			}
