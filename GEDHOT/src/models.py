...
class GEDIOT(torch.nn.Module):
    ...
    def forward(self,data):
        ...
        soft_matrix = map_matrix * cost_matrix # element-wise multiplication
...
class GEDGW(object):
    ...
    def set_cost_test(self):
        ...
        if self.n1 < self.n2:
            ...
            delta = self.n1-self.n2
            padding = (0, delta, 0, delta)
            self.cost2 = F.pad(self.cost2, padding, "constant", 0)
            padding1 = (0, delta, 0, 0)
            self.cross_cost = F.pad(self.cross_cost, padding1, "constant", 0)
            # Adjust mu to match the padded size
            self.mu = torch.ones(self.n1)
        ...
        max_size = max(self.n1, self.n2)
        self.nu = self.nu/max_size
        self.mu = self.mu/max_size
        self.cost1=self.cost1*max_size
        self.cost2=self.cost2*max_size
        self.cross_cost = (1-self.cross_cost)*max_size
